...
class viewticket
{
    ...
    public function display()
    {
		...
		{
			// Cambiamos el estado al ID 4 (o el que uses para Cerrado)
			// La función set_status ya se encarga de actualizar la tabla y añadir el historial
			$this->functions->set_status($ticket_id, 4); 

			// Eliminamos la línea que intentaba hacer el UPDATE a la columna inexistente 'ticket_closed'
        ...
        // --- LÓGICA DE ADJUNTOS PARA LA VISTA ---
        $attachments = [];
        ...
        // Procesamos adjuntos para el bloque global de ticket (Primer Mensaje)
        if (!empty($attachments[$ticket['post_id']]))
        {
            foreach ($attachments[$ticket['post_id']] as $attach)
            {
                // Definimos si es imagen para mostrar miniatura
                ...
                $this->template->assign_block_vars('ticket_attachments', [
                    'REAL_FILENAME' => $attach['real_filename'],
                    'U_DOWNLOAD' => $this->helper->route('nextgen_trackers_download', [
    'attach_id' => $attach['attach_id']
        ...
        // Cargamos historial de mensajes e inyectamos el array de adjuntos para los bloques hijos
