...
class functions
{
	...
	/**
	 * Muestra el historial y respuestas del ticket en orden DESC.
	 * Corregido para evitar que el historial desaparezca al mezclar con posts.
	 */
	public function get_posts_history($ticket, $start, $attachments = [])
	{
		...
		// Obtenemos el rango de tiempo de los posts cargados en esta página
		...
		// Definir límites de tiempo para buscar historial relacionado
		// Si no hay posts, usamos un rango amplio para no perder el historial (ej: cambios de status)
		...
		// Si es la primera página, nos aseguramos de traer todo el historial futuro
		...
		while (count($posts) || count($history_entries) || $p || $h) {
			...
			// Mezclar posts e historial en orden descendente (más nuevo arriba)
