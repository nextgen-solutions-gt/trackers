...
namespace nextgen\trackers\includes;
...
class functions
{
    ...
    /** @var array Table names with prefix */
    protected $tables = [];
    ...
    public function __construct(\phpbb\auth\auth $auth, \phpbb\config\config $config, ContainerInterface $container, \phpbb\db\driver\driver_interface $db, \phpbb\language\language $language, \phpbb\controller\helper $helper, \phpbb\request\request $request, \phpbb\template\template $template, \phpbb\user $user, $php_ext, $table_prefix)
    {
        ...
        $this->tables = [
            ...
            'trackers_watch'        => $table_prefix . 'watch',
    ...
    {
        $sql = 'SELECT COUNT(DISTINCT t.ticket_id) AS total_count ' . $this->tickets_sql($tracker, $project_id, $ticket_status, true);
        $result = $this->db->sql_query($sql);
        $total_tickets = (int) $this->db->sql_fetchfield('total_count');
        $this->db->sql_freeresult($result);
    ...
    {
        $sql = 'SELECT t.ticket_id, t.status_id, t.ticket_private, t.ticket_title, t.ticket_reported, t.duplicate_id, t.assigned_user, t.assigned_group, t.timestamp_created,
            st.status_name, st.status_colour, st.ticket_duplicate, se.severity_name, se.severity_colour, c.component_name,
        ...
        while ($row = $this->db->sql_fetchrow($result)) {
            ...
            $this->template->assign_block_vars('tickets', [
                'S_NEW'         => ($row['status_id'] == $status_new),
                'S_PRIVATE'     => (bool) $row['ticket_private'],
                'U_VIEWTICKET'  => $this->helper->route('nextgen_trackers_ticket', [
                    'page'   => 'viewticket', 
                    'ticket' => (int) $row['ticket_id']
                ]),
                'U_DUPLICATE'   => ($row['duplicate_id']) ? $this->helper->route('nextgen_trackers_ticket', [
                    'page'   => 'viewticket', 
                    'ticket' => (int) $row['duplicate_id']
                ]) : '',                
                'ID'            => $row['ticket_id'],
                ...
                'COMPONENT'     => ($row['component_name']) ?: ucfirst($this->language->lang('UNKNOWN')),
                'ASSIGNED'      => $assigned_to,
                'TITLE'         => $row['ticket_title'],
                'S_IS_REPORTED' => (bool) $row['ticket_reported'],
                'U_REPORT'      => $this->helper->route('nextgen_trackers_ticket', ['page' => 'report', 'ticket' => (int) $row['ticket_id']]),
                'STATUS'        => ($row['status_name']) ?: $this->language->lang('UNCATEGORISED'),
                'STATUS_COLOUR' => $row['status_colour'],
                'SEV_NAME'      => ($row['severity_name']) ?: $this->language->lang('UNCATEGORISED'),
                ...
                'U_MOVE'        => $this->helper->route('nextgen_trackers_ticket', ['page' => 'moderate', 'action' => 'move', 'ticket' => (int) $row['ticket_id']]),
                'U_UNASSIGN'    => $this->helper->route('nextgen_trackers_ticket', ['page' => 'moderate', 'action' => 'unassign', 'ticket' => (int) $row['ticket_id']]),
                'IS_ASSIGNED'   => ((int) $row['assigned_user'] > 0),
            ]);
        }
        $this->db->sql_freeresult($result);
    }

    public function tickets_sql($tracker, $project_id, $ticket_status, $is_count = false)
    {
        $sql = 'FROM ' . $this->tables['trackers_ticket'] . ' t
        ...
        if ($ticket_status == constants::STATUS_OPEN) {
            $sql .= ' AND (st.ticket_closed = 0 OR t.status_id = 0)';
    ...
    public function get_ticket_details($tracker_id, $project, $ticket)
    {
        ...
        $ticket_details[$this->language->lang('STATUS')] = ($ticket['status_name']) ?: $this->language->lang('UNCATEGORISED');

        if ($ticket['ticket_duplicate'] && $ticket['duplicate_id'] > 0) {
            $ticket_details[$this->language->lang('STATUS')] .= ' » <a href="' . $this->helper->route('nextgen_trackers_ticket', ['page' => 'viewticket', 'ticket' => (int) $ticket['duplicate_id']]) . '" style="color: #ff0000">#' . (int) $ticket['duplicate_id'] . '</a>';
        }
    ...
    public function get_posts_history($ticket, $start, $attachments = [])
    {
        ...
        $sql .= ' ORDER BY p.post_timestamp DESC, p.post_id DESC';
        ...
        $has_more = ($post_count > $this->config['posts_per_page']);
        if ($has_more) {
            array_pop($posts);
            $post_count--;
        }

        $post_max_timestamp = ($post_count > 0) ? $posts[0]['post_timestamp'] : PHP_INT_MAX;
        $post_min_timestamp = ($post_count > 0) ? $posts[$post_count - 1]['post_timestamp'] : 0;

        if (!$start) $post_max_timestamp = PHP_INT_MAX;
        ...
        while (count($posts) || count($history_entries) || $p || $h) {
            if (!$p && count($posts)) $p = array_shift($posts);
            if (!$h && count($history_entries)) $h = array_shift($history_entries);
            ...
            if ($p && (!$h || ($p['post_timestamp'] >= $h['history_timestamp']))) {
                ...
                $can_quote = ($this->user->data['is_registered'] && $this->auth->acl_get('u_tracker_reply')) || $is_moderator || $is_team;
                ...
                $this->template->assign_block_vars('ticket_posts', [
                    ...
                    'U_EDIT'    => ($can_edit) ? $this->helper->route('nextgen_trackers_ticket', ['page' => 'posting', 'mode' => 'edit', 'post' => (int) $p['post_id']]) : '',
                    'U_DELETE'  => ($can_delete) ? $this->helper->route('nextgen_trackers_ticket', ['page' => 'posting', 'mode' => 'delete', 'post' => (int) $p['post_id']]) : '',
                    'U_QUOTE'   => ($can_quote) ? $this->helper->route('nextgen_trackers_ticket', [
                        'page'   => 'posting', 
                        'mode'   => 'reply', 
                ...
                if (!empty($attachments[$p['post_id']])) {
                    foreach ($attachments[$p['post_id']] as $attach) {
                        ...
                        $display_html = ($is_image) 
                            ? '<div class="inline-attachment"><dl class="file"><dt class="attach-image"><a href="' . $download_url . '" target="_blank"><img src="' . $download_url . '" class="postimage" alt="' . $attach['real_filename'] . '" /></a></dt><dd>' . $attach['real_filename'] . ' (' . ($attach['filesize'] / 1024 > 1024 ? sprintf("%.2f MB", $attach['filesize'] / 1048576) : sprintf("%.2f KB", $attach['filesize'] / 1024)) . ')</dd></dl></div>'
                            : '<dl class="file"><dt><i class="icon fa-paperclip fa-fw" aria-hidden="true"></i> <a class="postlink" href="' . $download_url . '">' . $attach['real_filename'] . '</a></dt><dd>(' . ($attach['filesize'] / 1024 > 1024 ? sprintf("%.2f MB", $attach['filesize'] / 1048576) : sprintf("%.2f KB", $attach['filesize'] / 1024)) . ')</dd></dl>';
    ...
    public function generate_stats($tpl_block_var, $timespan_start = -1, $timespan_end = -1, $tracker_id = 0)
    {
        ...
        $sql = 'SELECT t.project_id, COUNT(t.ticket_id) AS tickets_open, SUM(t.ticket_private) AS tickets_private
            FROM ' . $this->tables['trackers_ticket'] . ' t
            INNER JOIN ' . $this->tables['trackers_status'] . ' s ON s.status_id = t.status_id
            WHERE s.ticket_closed = 0
        ...
        foreach ($projects as $project_id => $project_data)
        {
            ...
            $this->template->assign_block_vars($tpl_block_var, [
                'U_PROJECT' => $this->helper->route('nextgen_trackers_page', ['page' => 'viewproject', 't' => (int) $tracker_id, 'p' => (int) $project_id]),
    ...
    public function get_team_users($project_id = 0)
    {
		...
		$sql_ary = [
			'SELECT' => 'u.user_id',
			'FROM'   => [
				$this->tables['trackers_project_auth'] => 'pa',
				$this->tables['users']                 => 'u',
			],
		...
		if ($project_id > 0)
		{
			$sql_ary['WHERE'] .= ' AND pa.project_id = ' . (int) $project_id;
		}
	...
	public function delete_attachment($attach_id)
    {
        ...
        // We apply casting (int) directly in the SQL string to silence the EPV warning.
        ...
        if ($row && !empty($row['physical_filename'])) {
            ...
            // We also apply casting (int) in DELETE
    ...
    public function add_log($ticket_id, $operation, $data = array())
    {
        global $user;
        ...
        $sql = 'INSERT INTO ' . $this->table_prefix . 'log' . $this->db->sql_build_array('INSERT', $sql_ary);
        $this->db->sql_query($sql);
    }
    
    public function notify_watchers($ticket_id, $action_type = 'update')
    {
        $notification_manager = $this->container->get('notification_manager');

        $sql = 'SELECT t.ticket_id, t.project_id, t.ticket_title, u.username as author_username
                FROM ' . $this->tables['trackers_ticket'] . ' t
        ...
        if ($ticket_data)
        {
            $notification_manager->add_notifications('nextgen.trackers.notification.type.ticket_update', [
                'ticket_id'       => (int) $ticket_data['ticket_id'],
                'project_id'      => (int) $ticket_data['project_id'],
                'ticket_title'    => $ticket_data['ticket_title'],
                'author_username' => $this->user->data['username'],
            ]);

            $sql = 'UPDATE ' . $this->tables['trackers_watch'] . '
                    SET notify_status = 1
                    WHERE ticket_id = ' . (int) $ticket_id . '
	...
	/**
	* Generate smilies for the tracker editor
	*
	* @param string $mode   'inline' or 'window'
	* @param int    $forum_id
	* @return void
	*/
	public function generate_smilies($mode, $forum_id = 0)
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			{
				// Verification of the smiley description
				$description = (isset($row['smiley_description'])) ? $row['smiley_description'] : (isset($row['emoticon']) ? $row['emoticon'] : '');

				$this->template->assign_block_vars('smiley', array(
					'SMILEY_CODE'    => $row['code'],
					'A_SMILEY_CODE'  => $this->db->sql_escape($row['code']),
					// FIX: We use the absolute URL of the forum to avoid problems with Symfony.
					...
					'SMILEY_DESC'    => (string) $description,
				));
			}
